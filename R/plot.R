 ...
 plot.haplovalidate<- function(blocks,cmh,label=TRUE){
    ...
    blocks[,groupcol := col.sub]
        for (j in unique(blocks$chr)){
            ...
            cmh[chr==j,plot(pos/1000000,as.numeric(score),pch=19,col="#414547",ylim=c(0,maxy),xlab="position Mb",ylab="cmh score",lwd=3,cex.axis=1.5,cex.lab=2,main=paste("sim ",no," merge chr",j,sep=""),xaxp=c(0,50,50))]
            ...
            for (k in na.exclude(unique(cluster.sub.ord$groupcol))){
                ...
                count <- count +  round(max(as.numeric(cmh[chr==j,score]),na.rm=TRUE),-1) * 0.01
            }
            dev.off()
        }
 }
